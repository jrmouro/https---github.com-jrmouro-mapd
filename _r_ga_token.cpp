...
//                endpoint));
        ...
        nonTaskEndpoints.add(endpoint.linearIndex(map.getColumn_size()), endpoint);
...
_r_ga_token::_r_ga_token(const _r_ga_token& other) :
map(other.map),
...
r_currentStep(other.r_currentStep){
    
    
    if(other.r_name != nullptr){
            
        r_name = new std::string(other._name);
...
void _r_ga_token::draw(const Render& render) const {
...
const _ga_agent& _r_ga_token::getAgentById(int agentId)const {
...
void _r_ga_token::writeHeader(std::ostream& fs) const {
...
void _r_ga_token::writeRow(std::ostream& fs) const {
...
unsigned _r_ga_token::getCurrentStep() const {
    return currentStep;
}

void _r_ga_token::stepping() {
...
const _map& _r_ga_token::getMap() const {
    return map;
}

const _r_stepMap& _r_ga_token::getStepMap() const {
    return stepMap;
}

unsigned _r_ga_token::numberOfpendingTasks()const {
    return pendingTasks.size();
}

unsigned _r_ga_token::getRunningTaskAmount()const {
    return runningTasks.size();
}

unsigned _r_ga_token::getAssignedTaskAmount()const {
    return assignedTasks.size();
}

unsigned _r_ga_token::getPendingTaskAmount()const {
    return pendingTasks.size();
}

unsigned _r_ga_token::getFinishedTaskAmount()const {
    return finishedTasks.size();
}

const _agent_energy_system& _r_ga_token::getAgent_energy_system() const {
    return agent_energy_system;
}

unsigned _r_ga_token::numberOfAgents()const {
    return agents.size();
}

_r_ga_token* _r_ga_token::getClone()const {
        return new _r_ga_token(*this);
}

std::string _r_ga_token::id() const {
    return "GAT";
}

const std::string& _r_ga_token::getName() const {
    return _name;
}

void _r_ga_token::setName(const std::string& name){
        this->_name = name;
    }

int _r_ga_token::energyExpenditure() const {
    int ret = 0;
...
bool _r_ga_token::isAgentDelivering(int agentId) const {
    return getAgentById(agentId).isGoingToDelivering();
}

bool _r_ga_token::isAgentPickuping(int agentId) const {
    return getAgentById(agentId).isGoingToPickuping();
}

bool _r_ga_token::isAgentAtTrivialPath(int agentId) const {
    return getAgentById(agentId).isAtTrivialPath();
}

bool _r_ga_token::isIdle()const {

    return pendingTasks.empty() && runningTasks.empty();

}

bool _r_ga_token::isLate()const {

    return !pendingTasks.empty();

}

bool _r_ga_token::liberateEndpoint(const _ga_agent& agent, const _site& endpoint) {
...
_ga_agent* _r_ga_token::getEndpointObstructor(const _ga_agent& agent, const _site& endpoint) {
...
bool _r_ga_token::updateAgentRestPathCloserEndpoint(_ga_agent& agent, const _site& closerEndpoint) {
    ...
    listNonTaskEndpoints([&closerEndpointIndexerAlgorithm, &vsite, &agent, &closerEndpoint, this](const _site & endpoint) {
        ...
        closerEndpointIndexerAlgorithm.epa_solve(*this->map.getEndpointsPathAlgorithm(), endpoint, closerEndpoint, vsite);
...
bool _r_ga_token::updateAgentTaskPath_pendingTask(_ga_agent& agent, int newTaskId, bool trivialPath){
    ...
    if(pendingTasks.has(newTaskId, pendingTask)){
...
//            assignTaskAgent.insert(std::pair<int, int>(pendingTask.id(), agent.id()));
            ...
            finishedTasks.add(pendingTask.id(), pendingTask);
            pendingTasks.del(pendingTask.id());
            assignTaskAgent.add(pendingTask.id(), agent.id());
...
//                assignTaskAgent.insert(std::pair<int, int>(pendingTask.id(), agent.id()));
                ...
                runningTasks.add(pendingTask.id(), pendingTask);
                pendingTasks.del(pendingTask.id());
                assignTaskAgent.add(pendingTask.id(), agent.id());
...
bool _r_ga_token::updateAgentTaskPath_resting(_ga_agent& agent, int newTaskId) {
    
    return updateAgentTaskPath_pendingTask(agent, newTaskId, true);

}

bool _r_ga_token::updateAgentTaskPath_going_to_resting(_ga_agent& agent, int newTaskId) {
    
    return updateAgentTaskPath_pendingTask(agent, newTaskId, false);
    
}

bool _r_ga_token::updateAgentTaskPath_pickuping(_ga_agent& agent, int newTaskId) {
...
//        assignTaskAgent.insert(std::pair<int, int>(currentTask->id(), agent.id()));
        ...
        pendingTasks.del(currentTask->id());
        runningTasks.add(currentTask->id(), *currentTask);
        assignTaskAgent.add(currentTask->id(), agent.id());
...
bool _r_ga_token::updateAgentTaskPath_rest_pickuping(_ga_agent& agent, int newTaskId) {
...
bool _r_ga_token::updateAgentTaskPath_going_to_pickuping(_ga_agent& agent, int newTaskId) {
...
bool _r_ga_token::updateAgentTaskPath_going_to_rest_pickuping(_ga_agent& agent, int newTaskId) {
...
bool _r_ga_token::updateAgentTaskPath_going_to_rest_pickuping_to_pickuping(_ga_agent& agent, int newTaskId) {
...
bool _r_ga_token::updateAgentTaskPath_delivering(_ga_agent& agent, int newTaskId) {
...
//        runningTasks.erase(currentTask_it);
        ...
        finishedTasks.add(currentTask->id(), *currentTask);
        runningTasks.del(currentTask->id());
...
bool _r_ga_token::updateAgentTaskPath_rest_delivering(_ga_agent& agent) {
...
bool _r_ga_token::updateAgentTaskPath_going_to_rest_delivering_to_delivering(_ga_agent& agent, int newTaskId) {
...
bool _r_ga_token::updateAgentTaskPath(
        int agentId,
        int newTaskId) {
...
void _r_ga_token::addPendingTask(const _task& task) {
    this->pendingTasks.add(task.id(), task);
    this->nonTaskEndpoints.del(task.getPickup().linearIndex(map.getColumn_size()));
    this->nonTaskEndpoints.del(task.getDelivery().linearIndex(map.getColumn_size()));
}

void _r_ga_token::listNonTaskEndpoints(const std::function<bool(const _site&) >& function)const {
...
//    }
    ...
    nonTaskEndpoints.list([&function](const std::pair<unsigned, _site> pair){
...
const _task* _r_ga_token::getPendingTaskById(int taskId) const {
    
    const _task ** ret = nullptr;
    
    pendingTasks.has_ptr(taskId, ret);
...
void _r_ga_token::listPendingTasks(const std::function<bool(unsigned, const _task&) >& function)const {
...
//    }
    ...
    pendingTasks.list([&index, &function](const std::pair<unsigned, _task> pair){
        
        return function(index++, pair.second);        
        
    });

}

void _r_ga_token::listAgents(const std::function<bool(_ga_agent&) >& function) {
...
void _r_ga_token::listConstAgents(const std::function<bool(unsigned, const _ga_agent&) >& function)const {
...
void _r_ga_token::error_site_collision_check() const {
...
unsigned _r_ga_token::GetMaxPlannedStep() const {
...
void _r_ga_token::error_edge_collision_check() const {
