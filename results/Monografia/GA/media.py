...
df = pd.read_csv('./test_result_TP_0_2_5.csv',sep=';')
...
df3 = df2.groupby(["mapFilename","taskFilename","token_name"]).agg({'current_step':np.median, 'energy expenditure':np.median}).reset_index()

df4 = df2.groupby(["mapFilename","taskFilename","token_name"]).agg({'current_step': ['mean', 'min', 'max'], 'energy expenditure': ['mean', 'min', 'max']}).reset_index()

idx = df2.groupby(["mapFilename","taskFilename","token_name"])['current_step'].idxmax()

max_scores = df2.loc[idx]
