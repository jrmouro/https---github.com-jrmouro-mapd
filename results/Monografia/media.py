...
def normalize(df):
    result = df.copy()
    for feature_name in ["current_step","energy expenditure"]:
        result[feature_name].astype(float)
        max_value = float(df[feature_name].max())
        result[feature_name] = df[feature_name] / max_value
    return result

df = pd.read_csv('./Monografia/small_result.csv',sep=';')
...
# df2["energy expenditure"] = df2["energy expenditure"].round(3)

# df3 = normalize_by_group(df2, ["mapFilename","taskFilename","token_name"])
